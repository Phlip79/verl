...
MODEL_REVISION=${MODEL_REVISION:-d468880b6ad3c6e0d21377ce7242adaea4cc884d}
...
MODEL=(
    ...
    actor_rollout_ref.model.revision=${MODEL_REVISION}
...
TRAINER=(
    # v0.7's auto mode selects the legacy worker for this topology. Native
    # HybridModel MTP is implemented by the V1 engine path.
...
########################### launch ###########################
"${PYTHON_BIN}" -m verl.trainer.main_ppo --config-path=config --config-name=ppo_megatron_trainer.yaml \
    "${ALGORITHM[@]}" \
    "${REWARD[@]}" \
    "${DATA[@]}" \
    "${MODEL[@]}" \
