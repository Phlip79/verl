...
# DAPO-style recipe on DAPO-Math-17k and AIME-2024.
#
# Run this script once on the head of an already-started Ray cluster. The actor
# topology starts from NVIDIA's 2-node x 8-H100 SFT topology.
#
# Required source baseline (Python 3.12):
#   Megatron-Bridge r0.6.0 branch (version >= 0.6.0)
#   Megatron-Core core_r0.19.0 branch (version >= 0.19.0)
#   Transformers >= 5.8.1, < 5.11
#   vLLM == 0.27.1
#
# The Megatron actor trains Lightning's native MTP block and synchronizes that
# block into vLLM's Nemotron-H MTP drafter for speculative rollout.
# This recipe uses BF16 for both. Quantized online actor-to-MTP-drafter weight
# synchronization is not supported by this port.

set -xeuo pipefail

# vLLM 0.27.1's V1 runner is the supported path for hybrid Nemotron-H MTP.
...
MTP_ROLLOUT_SPEC=${MTP_ROLLOUT_SPEC:-True}
# Start conservatively. Increase only after measuring acceptance rate and
# throughput for the post-trained checkpoint.
...
MODEL=(
    ...
    "actor_rollout_ref.model.mtp.enable_rollout=${MTP_ROLLOUT_SPEC}"
    ...
    "actor_rollout_ref.model.mtp.num_speculative_tokens=${MTP_NUM_SPECULATIVE_TOKENS}"
)
