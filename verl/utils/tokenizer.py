...
def normalize_token_ids(tokenized_output) -> list[int]:
    ...
    ``apply_chat_template(tokenize=True)`` returns direct IDs in Transformers 4,
    but may return a ``BatchEncoding``/mapping in Transformers 5.
    """
...
def hf_processor(name_or_path, **kwargs):
    ...
    Returns:
        ...
        Returns ``None`` for text-only models whose AutoProcessor falls back to
        a tokenizer backend.
    """
